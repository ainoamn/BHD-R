...
## دورة المعاملة (إيجار / بيع) — معتمدة 2026-08-25

المرجع الكامل: [`implementation/TRANSACTION-FLOW-MAP.md`](./implementation/TRANSACTION-FLOW-MAP.md) **v1.3** (+ HTML).

| ID | القرار |
|----|--------|
| R1 | إلغاء بعقد: طلب → إدارة+تاريخ → محاسب (لا متأخرات/فواتير) → ملغي للمستأجر |
| R2 | التأمين مرتبط بكل خروج |
| R3 | تجديد: ملحق موقّع → شيكات/فواتير → محاسب (أو استثناء مدير) |
| R4 | بيع ناجح = نقل ملكية داخل النظام + مالك سابق في السجل |
| R5 | بيع وحدة مؤجرة مسموح مع نقل حقوق الإيجار للمالك الجديد |
