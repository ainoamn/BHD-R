...
| `/ar/owner/properties/:id` | **مركز عمليات** فقط (إحصائيات، تنبيهات، أزرار) — بدون تخطيط صفحة العرض العام |
| تعديل العقار → الوحدات | **مبلغ العربون / الحجز** (كان يُسمّى التأمين) — يُحفظ مع الوحدة ويُستخدم في «احجز الآن» |
| أقسام ops | `?propertyId=` يصفّي العقود / التأجير / البيع / الحجوزات / الصيانة / الفواتير لهذا العقار |

## مسارات API الجديدة (Vercel → Neon)

- `PATCH /api/owner/properties/:id/deposit` — حفظ مبلغ العربون (ما زال متاحاً؛ الإدخال الأساسي من معالج التعديل)
...
3. [إدارة عقار](https://r.bhd-om.com/ar/owner/properties) — «إدارة العقار» → hub؛ عيّن العربون من **تعديل العقار → الوحدات**.  
