# مصفوفة التحقق الأمنية والتقنية — متطلباتك مقابل BHD R 0.1.6
...
|   2 | فرض صلاحيات الوحدات والأدوار مركزياً على جميع APIs         | ✅      | `packages/authz`؛ `AuthenticationGuard` + `PermissionGuard` في `AppModule`                           | `route-policy.test.ts`؛ `security-guards.test.ts`؛ مصفوفة 90 صلاحية | deny-by-default                                                    |
...
|  11 | تحسين عزل الشركات + اختبارات منع الوصول بين المستأجرين     | ✅      | `organization_id` إلزامي؛ RLS في migrations؛ اختبار تكامل PostgreSQL بحساب non-superuser             | `packages/db/test/rls.integration.test.ts`                          | دفاع مزدوج: app + DB                                               |
...
|  13 | اختبارات Backend وFrontend وIntegration وE2E               | ✅      | Vitest للحزم/API/Web/Worker؛ Playwright 22 رحلة؛ API e2e                                             | `pnpm test` / `test:e2e` / CI                                       | وسّع تغطية الدفع الحقيقي في staging                                |
...
بالنسبة لبنودك القابلة للتنفيذ في المصدر: **BHD R 0.1.6 صُمم ونُفِّذ لإغلاقها بنيوياً** مع اختبارات ووثائق تشغيل.
