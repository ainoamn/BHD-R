...
- Merge owner/developer bookings into one control screen (`/bookings`): tabs for all / daily stays / monthly / yearly / sale; redirect `/stays/bookings` â†’ `/bookings?tab=daily`; remove duplicate stays nav item.

## 0.4.84 â€” 2026-09-11

- Fix owner `/properties` multi-minute blank: 6s Neon statement_timeout + 7s race, lighter property list (no per-unit cover fan-out), loading/error UI instead of silent empty, paced ops warm. Logout lands on `/ar` (not `/`â†’flash).
...
- Fix multi-minute soft-nav from homepage to `/properties`, `/units/[id]`, and `/portal`: 6â€“8s Neon timeouts, throttled catalogue heal, larger shared DB pool, deferred marketing prefetch of heavy routes.

## 0.4.80 â€” 2026-09-09

- Owner accounting: entry details (journal lines / stay payment), print slip, print filtered statement, CSV export; Hisaby link for full GL â€” not a bhd-om port.
...
- Confirmation page: show آ«طھظˆظ‚ظٹط¹ ط¹ظ‚ط¯ ط§ظ„ط¥ظ‚ط§ظ…ط©آ» when paid but unsigned; hide it once e-sign is completed; stop forcing redirect away from confirmed.

## 0.4.62 â€” 2026-09-03

- Re-enable post-pay e-sign (pay â†’ sign â†’ electronically approved contract â†’ confirmation); gate confirmed until signed.
...
- Stay calendar: day cells show ط´ط§ط؛ط± / ظ…ط­ط¬ظˆط² / ظ…ط¤ظ‚طھ / ظ…ط؛ظ„ظ‚ with clearer colors and a colored legend.
...
- Fix stays setup pricing step: serialize bigint rate fields in setup API JSON (was 500 آ«طھط¹ط°ط± طھظ†ظپظٹط° ط¥ط¬ط±ط§ط، ط§ظ„ط¥ط¹ط¯ط§ط¯آ»).
...
- Stays setup: Neon read/write on Vercel (`/api/owner/stays/setup`) â€” save & publish no longer depend on Nest/Render timeouts.
...
- Property manage hub: آ«ط¥ط¹ط¯ط§ط¯ ط§ظ„ط¥ظ‚ط§ظ…ط© ط§ظ„ظٹظˆظ…ظٹط©آ» as a prominent action button.
...
- Functional 5-step setup wizard (units â†’ capacity â†’ pricing â†’ content â†’ publish).
- Docs: `RELEASE-0.4.19-AR.md` â€” pilot enablement on Vercel + Render.
...
- Docs: `RELEASE-0.4.17-AR.md` â€” requires Vercel Redeploy without cache + S3 or Nest Live.
...
- Reviews: submit via Next `/api/public/reviews` + Vercel CSRF (`browserNextMutation`) â€” was wrongly calling Nest `browserMutation` and always failing as â€œsign in againâ€‌.
...
- Ops stay booking cancel and no-show: `POST /v1/stays/bookings/:id/cancel` and `â€¦/no-show` release inventory locks and cancel pending stay payment intents.
- Owner/developer bookings table actions (cancel / no-show / check-out) gated by status.
- Domain: allow `confirmed â†’ no_show`.
...
- Checkout â€œPay nowâ€‌ redirects to sandbox simulator; confirm reuses stay_booking confirmation path.
...
- Guest interactive stay checkout on `/stays/[slug]`: availability â†’ quote â†’ hold â†’ booking/payment intent via BFF.
...
- Payment webhook Expandâ€“Contract: `kind: stay_booking` + `paymentIntentId` confirms `stay_bookings` (amount/currency match, unique event id, holdâ†’booking lock, `stay_payment` journal) without changing invoice / reservation_deposit handlers.
...
- BHD R Stays (daily stays) phases 0â€“8 foundations behind closed Feature Flags: `stay_*` schema + GiST locks, domain pricing/booking machine, Nest StaysModule fail-closed, owner/developer portal shells, public `/stays` + homepage tab gated, guest trips shell, worker job stubs.
...
- Stays Phase 0 (branch): ADR-010, threat model, closed feature flags, regression baseline â€” **no public stays UI** (`RELEASE-STAYS-PHASE-0-AR.md`).

## 0.3.9 â€” 2026-08-31

- Properties browse: view toggle â€” list / grid / table (persisted in localStorage).
...
- Stop 3s flash of false آ«ط£ط¹ط¯ طھط³ط¬ظٹظ„ ط§ظ„ط¯ط®ظˆظ„آ» banner: optimistic empty payload, suppress status until hydrated, Nest-asleep offline no longer marks apiOnline false.
...
- Owner portal nav: Neon-first ops (approvals, invoices, expenses, maintenance + existing lists); skip Nest context/health when DB answers; empty offline shell if Nest asleep; never infinite آ«ط¬ط§ط±ظٹ طھط­ظ…ظٹظ„ ط§ظ„ظ‚ط³ظ…آ».
...
- Nest public booking checkout `POST /v1/public/units/:id/booking-checkouts` (throttled, honeypot, idempotent session replay) + Next Nest-first BFF â€” closes the last P0-02 public write gap; Neon remains fallback until Render catches up.
- Docs: `RELEASE-0.2.88-AR.md`.

## 0.2.87 â€” 2026-08-30

- Nest full property UPDATE bundle (`PATCH /v1/portfolio/properties/:id` matches wizard `{property,units}`) with `@Idempotent`; Next Nest-first BFF with Neon fallback â€” closes remaining owner property write gap for P0-02.
- Docs: `RELEASE-0.2.87-AR.md`.

## 0.2.86 â€” 2026-08-30

- Nest-first property create + media upload BFF (intentâ†’ingressâ†’complete) with Neon fallback; owner write rate limits; PATCH property Neon idempotency; ensure `CRON_SECRET` on Vercel when missing.
...
- Property wizard: آ«ط¥ط²ط§ظ„ط© ط§ظ„طµظˆط±ط©آ» now DELETE `/api/owner/media/:id` so existing gallery photos are removed from Neon (not only local React state).
...
- Share copy: polished listing blurb + description + â€œvisit the linkâ€‌ (AR/EN) instead of a dry title+URL.
...
- Rename unit **ط§ظ„طھط£ظ…ظٹظ†** â†’ **ظ…ط¨ظ„ط؛ ط§ظ„ط¹ط±ط¨ظˆظ† / ط§ظ„ط­ط¬ط²** in property edit (units step), with checkout hint; remove deposit editor from manage hub (set it in Edit property settings).
...
- Owner property page is an **ops hub** (stats, alerts, deposit, action buttons) â€” no public listing layout.
- Ops sections accept `?propertyId=` to show only that propertyâ€™s contracts / leasing / sales / bookings / maintenance / invoices.
- Catalogue cards show market-status watermark (available / for rent / for sale / reserved / leased / sold).
- Public listing CTAs: **ط·ظ„ط¨ ظ…ط¹ط§ظٹظ†ط©** (login required) and **ط§ط­ط¬ط² ط§ظ„ط¢ظ†** â†’ deposit checkout (`/book/:unitId`); owner sets deposit on manage hub.
...
- Owner portfolio: each property row shows **ط¹ط±ط¶ ط§ظ„ط¹ظ‚ط§ط±** (public listing) and **ط¥ط¯ط§ط±ط© ط§ظ„ط¹ظ‚ط§ط±** (Property 360). Manage page adds an Operations strip with Edit + links to contracts, leasing, sales, bookings, maintenance, and invoices.
...
- Replace the circular serif â€œRâ€‌ placeholder with the official BHD wordmark + R badge (`BrandMark`) in empty states, loading, listing/gallery placeholders, and ops empty tables. See [`docs/ASSETS.md`](./docs/ASSETS.md).

## 0.2.64 â€” 2026-08-30

- Fix public catalogue: sync `listings.enabled`/`publishedAt` when آ«ط¹ط±ط¶ ط§ظ„ظˆط­ط¯ط© ط¹ظ†ط¯ طھظˆظپط±ظ‡ط§آ» is saved; heal on Property 360 open; `/properties` and home load listings from Neon when Nest is empty.

## 0.2.63 â€” 2026-08-30

- Owner Property 360: آ«ط¹ط±ط¶ ط§ظ„ط¹ظ‚ط§ط±آ» opens the public listing; QR encodes `/[locale]/properties/:id` (read-only marketing view, no edit/ownership). Public gallery via `/api/public/media/:id`.
...
- Property photos: client-side compression + Vercelâ†’R2â†’Neon upload (with Nest fallback); gallery served via `/api/owner/media/:id`.
...
- Property 360 is full-width / mobile-friendly and **read-only**; editing only via آ«طھط¹ط¯ظٹظ„ ط§ظ„ط¹ظ‚ط§ط±آ» (wizard).

## 0.2.50 â€” 2026-08-30

- Property **edit** save uses Vercelâ†’Neon `PATCH /api/owner/properties/:id` (no Nest/Render), including address, owner, and units.
...
- **Break Nest dependency for property save:** wizard POSTs to `/api/owner/properties` (Vercel â†’ Neon) instead of Render Nest. Photos remain optional; media upload failures no longer wipe a successful save. Nest address `location` uses `ST_GeogFromText`.

## 0.2.47 â€” 2026-08-30

- Property wizard: images step is **optional** (no longer blocks continue/save). Clearer HEIC/iPhone rejection copy so empty gallery after â€œchoose photosâ€‌ is explained.
...
- Render: public Node edge still owns `PORT`/`/healthz`, but Nest Fastify **never listens**. All other routes (including `/raw-ping` and `/v1/*`) are dispatched with `fastify.inject()`. TCP listen/proxy left Nest â€œreadyâ€‌ while HTTP hung (blank `/raw-ping`, property save timeout).
...
- Early-bind raw Node HTTP on `PORT` before Nest (Render port scan + `/healthz`). Attach Fastify via `serverFactory` to the same server without a second `listen`. Avoid Nest `listen` + self-fetch deadlock that left HTTP hung after â€œlisteningâ€‌.
...
- Portal Nest â€œonlineâ€‌ uses `/healthz` (works on Live Render). `/api/warm` + cron + reconnect link switched off hanging `/health/ready`. Ops banner treats healthz OK as online even when Nest `/v1` context times out.
...
- Fix Render â€œNo open HTTP ports detectedâ€‌: Nest Fastify now binds with `listen(port, '0.0.0.0')` and logs the bound address (previous object-form listen left Render unable to route â†’ 502 / hung deploys).
...
- Portal speed (WAZEN lessons): dashboard overview reads Neon directly on Vercel when `DATABASE_URL` is set â€” no Nest wait for `/owner` metrics.
- WAZEN-style `NavigationProgress` top bar; raise client `staleTimes` (60s/300s).
- Docs: `WAZEN-SPEED-LESSONS-AR.md`.

## 0.2.30 â€” 2026-08-26

- Docs: Cloudflare Workers project `bhd-r` is not the deploy path â€” Git builds fail by design without wrangler; disconnect Git and use Vercel+Render (`CLOUDFLARE-NOT-DEPLOY-PATH-AR.md`).

## 0.2.29 â€” 2026-08-26

- Property wizard media: appending photos keeps existing ones (no replace); add-more label, remove per image, max 12.
- Save warm check fails within ~8s (no 25â€“180s hang); clear success banner on error; Nest reconnect actions on save failure.

## 0.2.28 â€” 2026-08-26

- Portal flicker: shell auth via JWT (`requirePortalShell`, â‰¤900ms DB race) so nav/chrome stay mounted; slim route pending bar.
...
- Property save: fail fast when Nest is down (BFF upstream timeout, Arabic CSRF/Nest errors, warm check before save) instead of hanging ~160s on آ«Could not establish a secure requestآ».

## 0.2.26 â€” 2026-08-25

- Portal performance: NestKeepAlive `/api/warm`, drop per-nav health probe, SPA `Link`+prefetch in ops console, CSRF token reuse, parallel media uploads (أ—3), Next `staleTimes` for soft nav.
- Ops banner: clearer Nest-down copy + آ«Reconnect to Nestآ» (`/api/warm` then refresh); NEST-API-HOSTING troubleshooting.
- Docs: `docs/implementation/PORTAL-PERF-AR.md`.

## 0.2.25 â€” 2026-08-25

- Fix Nest/Render Docker build: CORS origin must be Nest sync resolver (`resolveCorsOrigin`), not Express-style callback â€” unblocks API deploys stuck since `5766f9a` and restores media-ingress save path.
- Docs: `docs/implementation/RELEASE-0.2.25-AR.md` (publish checklist), NEST-API-HOSTING, PROPERTY-WIZARD, README.

## 0.2.24 â€” 2026-08-25

- Fix property save `Failed to fetch`: media uploads via Nest ingress token (same-origin `/v1/media/ingress/...`) instead of browserâ†’S3; CSP connect-src allows Nest; clearer Arabic network errors; BFF `maxDuration` 60s; wizard warms API on open.
- Docs: PROPERTY-WIZARD-AR, NEST-API-HOSTING; Render env `PUBLIC_NEST_ORIGIN`.

## 0.2.23 â€” 2026-08-25

- Mobile portal: fix drawer sidebar stuck open (entrance `transform` animation + `â‰¤1100px` layout cascade overriding the drawer breakpoint).
...
- Property wizard redesign: numbered gated steps, required field tones, Oman govâ†’wilayatâ†’village cascade, expanded amenities + custom, media icons/cover, AI-assisted bilingual description, homepage listing preview.
...
- Identity token verify moved into `apps/web` (`verify-id-token.ts`): access_token HS256 bind â†’ `/oauth/userinfo` â†’ id_token HS256; null-tolerant userinfo; error detail in `?x=`.
...
- Docs: `BHD-PRODUCT-SSO-ADMIN` checklist â€” `bhd-r` flipped to `mode=sso` on Identity catalog.
...
- Parties/CRM module, default contract template, migrations `0003`â€“`0007`, expanded RLS/roles, and full verification evidence in `docs/V1-COMPLETION-REPORT-AR.md`.
- Production cutover still requires backup, migrate `0003`â€“`0007`, re-apply RLS/runtime roles, secrets, then API+Worker+Web canary smoke (not claimed done by this tag alone).
...
- Login brand panel: unified آ«ط¨ظˆط§ط¨ط© BHDآ» gateway copy (Arabic + English).
...
- Identity linking per آ§0.7/`identity_subject`; Nest `POST /v1/auth/identity/session` for web-origin cookies.
- Frozen app-switcher catalog, unified logout via end-session, footer آ«ط¨ط±ط§ظ…ط¬ظ†ط§آ» + admin entry.
- Docs: `BHD-PRODUCT-SSO-ADMIN`, unified login copy with آ§12.9, `BHD-R-SSO-COMPLIANCE`, updated identity setup.
