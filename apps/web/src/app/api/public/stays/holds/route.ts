import { createStayHoldSchema } from '@bhd-r/contracts';
import { createPublicStayHoldOnNeon } from '@/lib/public-stays-booking-neon';
import {
  stayBookingDbGuard,
...
export async function POST(request: Request) {
  ...
  const limited = assertRouteRateLimit({
    key: hashRateKey(['stay-hold', clientIp(request)]),
    limit: 15,
    windowMs: 60_000,
  });
  if (!limited.ok) {
  ...
  const parsed = createStayHoldSchema.safeParse(body);
  if (!parsed.success) {
    return stayBookingJson({ error: { code: 'invalid_body' } }, { status: 400 });
  }
  ...
  try {
    const payload = await createPublicStayHoldOnNeon(parsed.data, idempotencyKey);
